...
test.describe('ホーム画面の表示に関するテスト', () => {
    ...
    test('「人気記事」、「新着記事」、「ブックマーク」の3つのボタンが表示され、最初は「人気記事」のボタンがアクティブになっている', async ({ page }) => {
        const popularArticleButton = page.getByRole('button').filter({ has: page.getByText('人気記事')})
