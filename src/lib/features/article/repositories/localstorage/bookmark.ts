...
const getBookmarkedArticlesId: IBookmarkRepository['getBookmarkedArticlesId'] = (): ArticleId[] => {
	...
	const bookmarkedArticlesId =
		(localStorage.getItem(BOOK_MARKED_ARTICLES_KEY) || '').split(',').filter(Boolean) ?? [];
	return bookmarkedArticlesId;
};

const setBookmarkedArticleId: IBookmarkRepository['setBookmarkedArticleId'] = (
	newBookmarkedArticleId: ArticleId
) => {
	const lastBookmarkedArticlesId =
		(localStorage.getItem(BOOK_MARKED_ARTICLES_KEY) || '').split(',').filter(Boolean) ?? [];
...
const deleteBookmarkedArticleId: IBookmarkRepository['deleteBookmarkedArticleId'] = (
	deletedBookmarkedArticleId: ArticleId
) => {
	...
	if (!lastBookmarkedArticlesId) {
		throw new NotFoundError(
			'articles',
			'NOT_FOUND_ARTICLES',
			'ブックマークした記事が見つかりませんでした'
		);
	}

	const newBookmarkedArticlesId = lastBookmarkedArticlesId.filter(
		(id) => id !== deletedBookmarkedArticleId
	);
