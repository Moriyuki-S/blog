...
const getFavoriteTagsId: IFavroteTagsRepository['getFavoriteTagsId'] = () => {
	...
	const favoriteTagsId =
		(localStorage.getItem(FAVORITE_TAGS_KEY) || '').split(',').filter(Boolean) ?? [];
...
const deleteFavoriteTagId: IFavroteTagsRepository['deleteFavoriteTagId'] = (
	deletedFavoriteTagId: TagId
) => {
	const currentFavoriteTagsId = getFavoriteTagsId();
	if (currentFavoriteTagsId.length === 0) {
		throw new NotFoundError('tags', 'NOT_FOUND_TAG', 'タグが見つかりませんでした');
	}
	const favoriteTagsIdAfterDeleted = currentFavoriteTagsId.filter(
		(tagId) => tagId !== deletedFavoriteTagId
	);
